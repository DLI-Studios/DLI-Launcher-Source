...
export function UpdateModal() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = launcherBridge.onMessage((msg: any) => {
      if (msg.type === 'UPDATE_PROGRESS') {
        ...
        setIsUpdating(false)
        setErrorText(msg.data?.error || 'Güncelleme sırasında bir hata oluştu.')
      }
    })
  ...
  const handleStartUpdate = async () => {
    setIsUpdating(true)
    setErrorText('')
    setStatusText('Güncelleme başlatılıyor...')
    ...
    if (!res.success) {
      setIsUpdating(false)
      setErrorText(res.error || 'Güncelleme başlatılamadı.')
    }
  }
        ...
        <div className="mb-6 flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground">Mevcut: v{updateInfo.currentVersion}</span>
            </div>
            <h2 className="text-2xl font-black italic tracking-wide text-white">Yeni Güncelleme Mevcut!</h2>
          </div>
        </div>
        ...
        {isUpdating ? (
          <div className="space-y-3 py-2">
            ...
            <div className="h-3 w-full overflow-hidden rounded-full bg-white/10 p-0.5 border border-white/10">
              <div
                className="h-full rounded-full bg-gradient-to-r from-primary via-purple-500 to-pink-500 transition-all duration-300 shadow-lg shadow-primary/50"
                style={{ width: `${progress}%` }}
              />
            </div>
            <p className="text-center text-xs text-muted-foreground">
              Güncelleme tamamlandıktan sonra DLI Launcher otomatik olarak yeniden başlatılacak.
            </p>
          </div>
        ) : (
          /* Butonlar */
          <div className="flex items-center justify-end gap-3 pt-2">
            ...
            <button
              ...
              className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-primary to-purple-600 px-6 py-2.5 text-sm font-bold text-white shadow-lg shadow-primary/30 hover:brightness-110 active:scale-95 transition-all"
            >
