import { Home, Play, Package, Sparkles, ShoppingCart, User, Settings, List, MessageCircle, AtSign, Clapperboard, Globe } from 'lucide-react'
    ...
    <aside className="flex h-full w-56 shrink-0 flex-col border-r border-sidebar-border bg-sidebar">
      ...
      <nav className="flex flex-col gap-1 px-3" aria-label="Main navigation">
        {navItems.map((item) => {
          ...
          return (
            <button key={item.id} type="button" onClick={() => onNavigate(item.id)} aria-current={isActive ? 'page' : undefined}
              className={cn('group relative flex items-center gap-3 rounded-lg px-4 py-3 text-sm font-semibold tracking-wide uppercase transition-all duration-200',
              ...
              {isActive && <span className="absolute right-0 top-1/2 h-6 w-0.5 -translate-y-1/2 rounded-full bg-primary shadow-[0_0_8px_2px] shadow-primary/60" />}
      ...
      <div className="mx-4 mb-4 rounded-xl border border-border bg-card/60 px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col"><span className="text-[10px] tracking-widest text-muted-foreground uppercase">All Systems</span><span className="text-xs font-bold text-foreground">OPERATIONAL</span></div>
        </div>
      </div>
