...
const CUSTOM_AVATAR_PREFIX = 'dli_custom_avatar_'
const MAX_AVATAR_SIZE = 128

function customAvatarKey(userId?: string): string {
  return userId ? `${CUSTOM_AVATAR_PREFIX}${userId}` : 'dli_custom_avatar'
}
...
class PlayerService {
  ...
  getCustomAvatarUrl(): string | null {
    ...
    return localStorage.getItem(customAvatarKey(user?.id))
  }

  setCustomAvatarUrl(url: string) {
    const user = authService.getUser()
    localStorage.setItem(customAvatarKey(user?.id), url)
    this.invalidateCache()
  }

  clearCustomAvatar() {
    const user = authService.getUser()
    localStorage.removeItem(customAvatarKey(user?.id))
    this.invalidateCache()
  }
