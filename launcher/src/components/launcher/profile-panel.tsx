...
export function ProfilePanel({ onLogout }: ProfilePanelProps) {
  ...
  const loadPlayer = async () => {
    const p = await playerService.getPlayer()
    setPlayer(p)
  }

  useEffect(() => {
    loadPlayer()
    const unsubscribe = authService.onAuthChange(() => loadPlayer())
    return unsubscribe
  }, [])

  if (!player) {
    return (
      <section className="glass shrink-0 rounded-xl border border-border p-4 animate-pulse" aria-label={t('aria.playerProfile')}>
  ...
  return (
    <section className="glass shrink-0 rounded-xl border border-border p-4" aria-label={t('aria.playerProfile')}>
      <div className="flex items-center gap-4">
        {player.avatarUrl ? (
          <img
        ...
        <div className="flex flex-col gap-1 min-w-0">
          <span className="flex items-center gap-1.5 text-lg font-bold text-foreground truncate">
            {player.username}
            <BadgeCheck className="size-4.5 shrink-0 text-accent" aria-label={t('common.verified')} />
          </span>
          {player.isPremium && (
