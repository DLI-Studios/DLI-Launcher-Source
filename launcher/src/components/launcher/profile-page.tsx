...
export function ProfilePage({ onLogout }: ProfilePageProps) {
  ...
  const handleAvatarSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith('image/png')) {
      setUploadError({ code: 'ERR_AVATAR_FORMAT', message: t('profile.avatarFormat') })
      return
    }

    if (file.size > 2 * 1024 * 1024) {
      setUploadError({ code: 'ERR_AVATAR_SIZE', message: t('profile.avatarSize') })
      return
    }
    ...
    try {
      ...
      if (err?.message === 'TIMEOUT') {
        setUploadError({ code: 'ERR_AVATAR_TIMEOUT', message: t('profile.avatarTimeout') })
      } else if (err?.message === 'No authenticated user') {
        setUploadError({ code: 'ERR_AVATAR_AUTH', message: t('profile.avatarAuth') })
      } else {
        setUploadError({ code: 'ERR_AVATAR_STORAGE', message: t('profile.avatarStorage') })
      }
    } finally {
      setUploading(false)
  ...
  return (
    <div className="flex flex-col h-full p-6 gap-5 overflow-y-auto">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-black tracking-widest text-foreground uppercase">{t('profile.title')}</h1>
        <button
          type="button"
          onClick={onLogout}
        ...
        <div className="flex items-center gap-5">
          ...
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-2">
              <h2 className="text-2xl font-bold text-foreground">{user?.global_name || user?.username || t('common.unknown')}</h2>
      ...
      <div className="grid grid-cols-4 gap-3">
        <div className="rounded-xl border border-border bg-card/50 p-4 text-center">
          ...
          <span className="text-[10px] text-muted-foreground">{t('common.dliCoins')}</span>
        </div>
        <div className="rounded-xl border border-primary/30 bg-primary/10 p-4 text-center">
          ...
          <span className="text-[10px] text-muted-foreground">{t('common.dliGems')}</span>
        </div>
        <div className="rounded-xl border border-border bg-card/50 p-4 text-center">
          ...
          <span className="text-[10px] text-muted-foreground">{t('profile.playTime')}</span>
        </div>
        <div className="rounded-xl border border-border bg-card/50 p-4 text-center">
          <Shield className="size-6 text-muted-foreground mx-auto mb-2" />
          <span className="text-xl font-bold text-foreground block">12</span>
          <span className="text-[10px] text-muted-foreground">{t('profile.achievements')}</span>
        </div>
      </div>

      {/* Account Info */}
      <div className="rounded-xl border border-border bg-card/50 p-4 space-y-3">
        <h3 className="text-sm font-bold text-foreground">{t('profile.accountInfo')}</h3>
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-between py-2 border-b border-border">
            <span className="text-xs text-muted-foreground">{t('profile.displayName')}</span>
            <span className="text-xs font-semibold text-foreground">{user?.global_name || user?.username || t('common.unknown')}</span>
          </div>
          <div className="flex items-center justify-between py-2 border-b border-border">
            ...
            >
              {user?.id || t('common.unknown')}
              {copied ? <Check className="size-3 text-success" /> : <Copy className="size-3 text-muted-foreground" />}
            </button>
          </div>
          <div className="flex items-center justify-between py-2">
            <span className="text-xs text-muted-foreground">{t('profile.minecraftUsername')}</span>
            <span className="text-xs font-semibold text-foreground">{player?.username || t('profile.notLinked')}</span>
          </div>
        </div>
      </div>
      ...
      {/* Quick Links */}
      <div className="rounded-xl border border-border bg-card/50 p-4 space-y-3">
        <h3 className="text-sm font-bold text-foreground">{t('profile.quickLinks')}</h3>
          ...
          <div className="mx-4 w-full max-w-sm rounded-2xl border border-destructive/30 bg-card p-6 shadow-2xl shadow-destructive/10">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-sm font-bold text-foreground">{t('profile.uploadFailed')}</h3>
